...
struct axes_have_value_policies<Axes, utils::void_t<typename Axes::value_policy>> : std::true_type {
};
template <typename Axes>
...
//@{
template <typename T, typename Policies, std::size_t N, typename = utils::void_t<>>
struct nth_policy {
    using type = no_change<T>;
};
template <typename T, typename Policies, std::size_t N>
struct nth_policy<T, Policies, N, std::enable_if_t<(N < Policies::size)>> {
    using type = typename Policies::template type<N, T>;
};
template <typename T, typename Policies, std::size_t N>
using nth_policy_t = typename nth_policy<T, Policies, N>::type;
//@}
...
struct value_policies {
    ...
    template <std::size_t>
    using value_policy = no_change<T>;
};

template <typename T, typename Axes>
struct value_policies<T, Axes, std::enable_if_t<axes_have_value_policies_v<Axes>>> {
    using axes_value_policy = typename Axes::value_policy;
    template <std::size_t N>
    using value_policy = nth_policy_t<T, axes_value_policy, N>;
};
